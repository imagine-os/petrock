...
Every surface a machine (script, agent, voice controller, MCP client) can drive, recorded every pass (principle P-10, D-203). Update this file in the same turn as any change to a route, `DataProvider` method, npm script, action or API. Last full pass: 2026-09-21 (prompt 0018).

## 1. What exists today

### 1.1 Route manifest (in the running app)

`src/app/manifest.ts` publishes `window.__petrock = { routes, version }` on load. Each entry: `{ path, code, surface, status: 'built' | 'stub', roles, spec }` where `spec` is the full `PageSpec` (`src/specs/types.ts`: code, name, purpose, layout, data, roles, logic, integrations, components, rules, states, notes, figma, checkedAt, tone). Consumers: `scripts/screenshots.mjs`, `scripts/qa-responsive.mjs`, `scripts/gen-specs.mjs`, the D-19 route manifest page (download as JSON), D-03 / D-09 spec pages. Routing is `react-router-dom` HashRouter, so every page is `/#/<path>`; parameterised routes use `:id`, `:table`, `:code`. `spec.actions?: ActionDef[]` now exists (D-221, shape in section 2.1) and is published with the rest of the spec; HUB-01 is the first page to declare actions (7): `hub.enterAs`, `hub.openTool`, `hub.setLang`, `hub.toggleTheme`, `hub.setBrand`, `hub.toggleDevMode` (permission `dev.tools`), `hub.switchLocation`. The public website added one route this pass, **`/site/gallery` (P-13, surface `public`, roles everyone)**, and six of its pages now declare actions (prompt 0017): P-01 has 8 (`site.book`, `site.callLocation`, `site.hero.playVideo`, `site.hero.pauseVideo`, `site.openService`, `site.openGallery`, `site.openLocations`, `site.openReviews`) and P-13 has 5 (`site.gallery.open`, `site.gallery.next`, `site.gallery.prev`, `site.gallery.close`, `site.book`); P-02 declares 4, P-03 2 and P-12 4. None of the site actions needs a permission (the surface is public). Every other page's `actions` is still empty and the bus that runs them is not built (section 2.1). The front desk's six hotel and grooming views declared their actions this pass (prompt 0018, D-197): **F-10** 8 (`reservations.switchView`, `reservations.newBooking`, `reservations.setDay`, `reservations.seeAll`, `reservations.filter`, `reservations.toggleGroup`, `reservations.openBooking`, `reservations.editBooking`); **F-13** 10 (`reservations.switchView`, `reservations.setWeek`, `reservations.setSpan`, `reservations.filter`, `reservations.openStay`, `reservations.collapseGroup`, `reservations.moveStay`, `reservations.assignRoom`, `reservations.setStatus`, `reservations.newBookingForCell`); **F-14** 9 (`reservations.switchView`, `grooming.setDay`, `grooming.setSpan`, `grooming.filterGroomer`, `grooming.filterPackage`, `grooming.openAppointment`, `grooming.moveStatus`, `grooming.collapseColumn`, `grooming.openDayView`); **F-30** 11 (`grooming.setDay`, `grooming.filterGroomer`, `grooming.filterStatus`, `grooming.openAppointment`, `grooming.bookSlot`, `grooming.reschedule`, `grooming.moveColumn`, `grooming.colourColumn`, `grooming.hideColumn`, `grooming.showColumn`, `grooming.newBooking`); **F-31** 6 (`grooming.setDay`, `grooming.filterGroomer`, `grooming.openAppointment`, `grooming.moveStatus`, `grooming.collapseColumn`, `grooming.newBooking`); **F-32** 6 (`grooming.setDay`, `grooming.filterGroomer`, `grooming.filterStatus`, `grooming.filterPayment`, `grooming.openAppointment`, `grooming.newBooking`). Write actions carry `bookings.write`, `bookings.status` or `appointments.write`; `grooming.setDay`, `grooming.filterGroomer`, `grooming.moveStatus` and `grooming.collapseColumn` are shared ids used by more than one page and must keep one meaning. Two library components were **retired** this pass and are gone from every spec's `components`: `GroomAppointmentCard` (folded into the new `ScheduleCard` molecule) and `GroomStatusBoard` (folded into `AppointmentBoard`) - an agent reading an older spec or doc should treat both names as historical (D-231). No route, `DataProvider` method or npm script changed in prompt 0018.
...
| `npm run site:images` | `node scripts/site-images.mjs`: the public-website photo pipeline. Input `docs/reference/petrockhotel-scrape/images` (the 50 crawled originals), output `public/site/img` (152 committed derivatives: WebP 480 / 960 / 1600 / 2560 never upscaled, a 1600 JPEG fallback, <= 450 KB each, plus the logo PNG) and the generated manifest `src/modules/extras-manual-website/site/siteImages.generated.ts` (D-225). **sharp is a devDependency**, so this never runs in CI - the derivatives are committed | none |
...
- 2026-09-20 (prompt 0017): new public route `/site/gallery` (P-13); spec actions declared on P-01 (8), P-02 (4), P-03 (2), P-12 (4) and P-13 (5); new `npm run site:images` script row (D-225); no `DataProvider` or API change.
- 2026-09-21 (prompt 0018): spec actions declared on F-10 (8), F-13 (10), F-14 (9), F-30 (11), F-31 (6) and F-32 (6); `GroomAppointmentCard` and `GroomStatusBoard` retired in favour of `ScheduleCard` / `AppointmentBoard` (D-231); no route, `DataProvider`, script or API change.
