...
- `R-X66` - see `src/rules/` (registry) and the Rules tab of the inspector on this page
...
- Assign upserts conversation_assignments (R-X66)
...
- 0019 - QA fixes: Staff replies notify the customer with a deep link to `/app/inbox/<conversation>`; seed thread timestamps are relative so a fresh reply sorts last.
