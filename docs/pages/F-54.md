...
- `R-X35` - see `src/rules/` (registry) and the Rules tab of the inspector on this page
...
- Certificate + dates by a verifier = verified; dates only = submitted; none = missing (R-X35)
...
Real: pet + profile + vaccine records, approval recomputed (settleVaccineStatus). Mock: certificate files are mock:// URLs; desk-entered certificates are verified on entry (R-X35).
