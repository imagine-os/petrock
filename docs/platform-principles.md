...
- _Today_: the QA matrix stops at 1920 (`scripts/qa-responsive.mjs`, `WIDTHS`); the customer app is 390-first, the desk 1440-first; 12 px is the text floor (D-175). TV widths may be verified on the key pages first (HUB-01, C-10, C-30, F-01, F-10, F-12, A-01, P-01) and then across the matrix; HUB-01 is the first page verified to 2560 / 3840 through its own `--hub-scale` band (D-220, prompt 0016).
...
- _Today_: the library components carry a11y notes (D-08 matrix, R-X81); the D-15 scan checks names, labels, heading order, targets. Drag exists in D-11 layout editor (has up / down buttons) and in the room timeline (assignment picker as alternative). Since prompt 0018 every board and grid card carries one 44 px "more" menu that offers the same status moves as dragging, so drag is an accelerator only (D-234), and the timeline popover opens on click, Enter or hover (D-232). Known leftovers under 44 px: the `DataTable` sort buttons, the `GroomingDateNav` "Today" label and the visually hidden checkbox inputs - all pre-existing and carded in the kanban.
...
- _Today_: `PageSpec.actions?: ActionDef[]` exists in the D-197 shape and HUB-01 declares its 7 actions (D-221, prompt 0016); every other page still just calls `can('...')` and renders buttons, and nothing runs an action yet. The route manifest (`src/app/manifest.ts`) exposes specs, and now the actions, to tooling.
...
- _Today_: 74 tables, 137 components with metas, D-01..D-19 dev pages; this is already the contract in `CLAUDE.md`. The public website uses the library for its photos too: `SitePhoto` (`<picture>` + srcset, intrinsic size, lazy unless `priority`) is a molecule with a meta, so no page hand-rolls an `<img>` (D-225, prompt 0017). Prompt 0018 collapsed two duplicate pairs into one each: the new `ScheduleCard` molecule is the only appointment / booking card and `AppointmentBoard` the only Kanban organism, with `GroomAppointmentCard` and `GroomStatusBoard` deleted (D-231) - when two components drift apart, the fix is one component with a variant, not a second copy.
...
- _Today_: the `Placeholder` atom exists (D-222, prompt 0016) with a tooltip, a "not wired yet" toast, a dev-mode outline and `data-placeholder`, and the hub's three unbuilt tools (Canvas D-21, Demo simulator D-22, Plan viewer D-23) are its first consumers; `PageStub` still renders its own stub card and five `onClick`s still toast "coming soon" (HomePage, PetProfilePage, ...).
