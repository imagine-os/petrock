...
- 4K / TV QA pass (P-01, D-194): add 2560 and 3840 to `scripts/qa-responsive.mjs` WIDTHS and D-12 / D-13, legibility check (body >= 16 px at >= 1920), `--scale` token band at >= 2560, focus ring token; verify key pages first (HUB-01, C-10, C-30, F-01, F-10, F-12, A-01, P-01), then the full matrix; record 2560 / 3840 in `spec.checkedAt`
- `Placeholder` atom + dev-mode toggle (P-09, D-202): `src/components/atom/Placeholder/` with meta, tooltip + "not wired yet" toast, dashed outline in `devMode`, `data-placeholder`; wire into `PageStub` and every toast-only `onClick` (HomePage, PetProfilePage, ...); D-09 counts placeholders
- `actions` manifest on `PageSpec` + `/#/dev/actions` (D-20) (P-05, D-197): `ActionDef { id, label, intent, permission?, params? }`, actions bus in `src/actions/` with `run()` handlers registered while mounted, `specCompleteness` check, D-20 page listing every action; fill the manifest module by module (reservations F-10..F-15 first); record in `docs/reference/surfaces.md`
