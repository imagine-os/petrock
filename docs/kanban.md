...
- Regenerate C-3x / C-5x / C-6x / F-1x / A-2x / P-05 screenshots after the price change (values in the old captures are stale)
- Fidelity follow-ups (from `docs/design/fidelity/README.md`): rebuild C-33 to the Choose Pet / Choose Package two-up (`Frame 1171276428.png`, needs Justin on the flow); replace the `svc-*` / `nav-*` provisional glyphs 1:1 from the Figma Design System page export (API quota back ~Sep 22, D-210); licensed hero / pet / room photos for `HomeHero`, `PetAvatarCard`, `HotelRoomTypeCard` (D-214, open question 116); C-30 share-room / Location as text inputs if Justin confirms; F-13 grey only the group label cell
- 4K / TV QA pass (P-01, D-194): add 2560 and 3840 to `scripts/qa-responsive.mjs` WIDTHS and D-12 / D-13, legibility check (body >= 16 px at >= 1920), `--scale` token band at >= 2560, focus ring token; verify key pages first (HUB-01, C-10, C-30, F-01, F-10, F-12, A-01, P-01), then the full matrix; record 2560 / 3840 in `spec.checkedAt`; hub-scoped band landed for HUB-01 (D-220); remaining: global token + matrix
- `Placeholder` atom + dev-mode toggle (P-09, D-202): `src/components/atom/Placeholder/` with meta, tooltip + "not wired yet" toast, dashed outline in `devMode`, `data-placeholder`; atom landed in 0030 (D-222); remaining: wire into `PageStub` and toast-only `onClick`s, D-09 count
- `actions` manifest on `PageSpec` + `/#/dev/actions` (D-20) (P-05, D-197): `ActionDef { id, label, intent, permission?, params? }`, actions bus in `src/actions/` with `run()` handlers registered while mounted, `specCompleteness` check, D-20 page listing every action; `ActionDef` type + HUB-01's 7 actions landed in 0030 (D-221); remaining: bus, D-20 page, `specCompleteness` check, module fill (reservations F-10..F-15 first); record in `docs/reference/surfaces.md`
- Canvas D-21 `/dev/canvas`, Demo simulator D-22 `/dev/simulator`, Plan viewer D-23 `/plan` (hub placeholders, D-222): CTL has reference implementations; plan viewer needs a task source (kanban.md -> tasks.json?)
- Promote `hub.role.*` and `placeholder.*` strings to a shared string table (Spanish-fill pass)
- Annotations module (P-08, D-200): extend `feedback` (kind, element_path, component, viewport, theme, screenshot_url, triage, triage_note, decision_ref), element picker in `FeedbackButton`, pins on page for dev mode / `feedback.read`, A-36 inbox columns + filters, `docs/reference/annotations-triage.md` (agent workflow: read store, fix vs ask by author, record decision first)
- `docs/reference/surfaces.md` kept current every pass (P-10, D-203): add the actions registry, WebMCP tool list and CLI when they land; check on every route / provider / script change
- Spanish fill pass (P-13, D-206): translate the `es` side of every module's `strings`, replace hard-coded English in JSX with `useT()` keys, strings-coverage check in the D-09 spec report; Sonnet-class mechanical pass
- Spatial-navigation (d-pad) focus manager spike (P-04, D-195): `useSpatialNav` in `src/a11y/`, arrow keys / Gamepad API move focus by geometry, opt-in per shell, no focus traps; try on HUB-01 and F-01
- Presence / realtime plan doc tied to Company-OS (P-14, D-207): `docs/reference/realtime-plan.md` (presence table, `version` base column, conflict UI, offline queue, `DataProvider.subscribe` over Company-OS channels); "what Petrock expects from Company-OS" section in `docs/reference/company-os.md`; no wiring until D-183 lifts
- Promote a11y `target-size` (44 px) from warning to error in `scripts/qa-lib.mjs` and D-15; hover-only affordance audit across the library (P-03, D-195)
...
- Company-OS integration Phase 0: generate entity template (`npm run companyos:template`), `.env.example`, provider chooser, module specs under `docs/specs/company-os-modules/` (blocked on Justin's answers to prompt 0011)
...
- Awaiting Justin: fidelity decisions D-188 (fonts), D-210 (icons), D-215 (four-tab nav), D-216 (desk table structure), D-213 desk badge hues (`front desk-9.jpg` pairs or lifecycle hues?); D-179 (PIN at check-in?), review of D-010/D-011, Figma paid seat or wait for Sep 22 (D-012); answers to the pricing conflicts chosen in the build (daycare 6 h, card fee 3.89 %, Diamond prices); owner question list (15 questions, prompt 0008 / `docs/reference/petrockhotel-site.md`) to take to the owner; **confirm the D-229 not-adopted list** (per-holiday "please call" prices, cat grooming, the large-dog accommodation fee, dematting, and the moveable season dates per year)
...
- Figma fidelity fix at the design-system level (Justin 2026-09-18, prompts 0009 / 0010 / 0012 / 0013 / 0015, changelog 0024-0029, **v0.2.0**): audit -> (a) fonts Open Sans / Be Vietnam Pro, tokens, 32 Figma icons, core components -> (b) customer screens rebuilt to the exports (C-10 first) -> (c) desk shell, framed tables, timeline grid, KPI cards -> (d) merged and measured (composites + scores in `docs/design/fidelity/README.md`: C-10-gate 0.38 -> 0.70, C-13 0.44 -> 0.58, C-30 0.41 -> 0.60, F-13 0.47 -> 0.59); decisions D-188..D-191, D-210..D-217; full screenshot set regenerated
- Platform principles recorded (prompt 0014, D-193..D-209): `docs/platform-principles.md` P-01..P-15, CLAUDE.md quality-bar section and rules (matrix to 3840, placeholders, actions manifest, en / es, surfaces.md), `docs/reference/surfaces.md` initial version, backlog cards for the code work
