...
- Awaiting Justin: fidelity decisions D-188 (fonts), D-210 (icons), D-215 (four-tab nav), D-216 (desk table structure), D-213 desk badge hues (`front desk-9.jpg` pairs or lifecycle hues?); **enable GitHub Pages** (Settings > Pages > Source = GitHub Actions, then re-run "Deploy to GitHub Pages"; runs #7 / #8 stopped at configure-pages: "Resource not accessible by integration"); D-179 (PIN at check-in?), review of D-010/D-011, Figma paid seat or wait for Sep 22 (D-012); answers to the pricing conflicts chosen in the build (daycare 6 h, card fee 3.89 %, Diamond prices); owner question list (15 questions, prompt 0008 / `docs/reference/petrockhotel-site.md`) to take to the owner
