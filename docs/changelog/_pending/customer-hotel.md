# customer-hotel - hotel stay booking, my reservations, detail / invoice / change requests
...
decision: docs/decisions/_pending/customer-hotel.md (D-CH-01..18; deposit 30 %, 48 h free cancellation, Suite copy and the 55 lb rule flagged needs Justin)
...
- **Rules**: `src/rules/customer-hotel.ts` adds R-X30..R-X39 (deposit %, initial status, change requests, separate rooms, stay validation, grooming at check-out, capacity, bottom rooms, card fee on the charged amount, average nightly rate), all `implemented`.
