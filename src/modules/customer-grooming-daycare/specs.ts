...
export const groomingStartSpec = defineSpec({
  code: 'C-50', name: 'Grooming & Spa', purpose: 'Entry point of the Grooming & Spa service for a pet parent: what it is, live price menu by package and size, spa add-ons, upcoming orders, and the Book now call to action (D-004 / D-011: one service, Spa is a package category).',
...
export const groomingPetPackageSpec = defineSpec({
  code: 'C-51', name: 'Choose pet & package', purpose: 'Step 1 of the Grooming & Spa order: pick the pet and the package; prices are the package table values for that pet\'s size tier.',
...
export const groomingAddonsSpec = defineSpec({
  code: 'C-52', name: 'Add-ons', purpose: 'Step 2: toggle spa add-ons for the current pet (price and added minutes by size), see the running total, and add another pet to the same order.',
...
export const groomingCheckoutSpec = defineSpec({
  code: 'C-54', name: 'Summary & payment', purpose: 'Step 4: review when / where / who, per-pet package and add-ons, pick card or pay at location, see engine totals (tax, card fee) and pay. Creates the order, appointments, invoice, payment and notification.',
...
export const groomingOrdersSpec = defineSpec({
  code: 'C-56', name: 'Your Grooming & Spa orders', purpose: 'History of Grooming & Spa orders split into upcoming and past, each re-creatable in one tap; entry to a new order.',
...
export const daycareStartSpec = defineSpec({
  code: 'C-60', name: 'Daycare', purpose: 'Entry point of Daycare: what it is, live pricing (full day, half day, play hour, walk with the hour threshold and the extra-pet discount), upcoming days and Book now. Replaces the "Coming soon" placeholder (R-A03).',
...
export const daycareDetailsSpec = defineSpec({
  code: 'C-62', name: 'Additional pet details', purpose: 'Step 2: per selected pet, vet-recommended flea medication (brand, last applied) and medical alerts for the handlers.',
...
export const daycareCheckoutSpec = defineSpec({
  code: 'C-63', name: 'Daycare checkout', purpose: 'Step 3: summary of day, times, location and pets, payment method, engine totals and pay. The Figma checkout was empty (DayCare-3), so this is designed to match the grooming checkout.',
