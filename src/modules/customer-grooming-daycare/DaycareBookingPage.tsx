  ...
  return (
    <CgdPage title={isNew ? 'You\'re booked!' : `Daycare ${booking.code}`} backTo={isNew ? '/app/daycare' : '/app/daycare/bookings'}>
      ...
      <div className="cgd-inline-actions">
        ...
        {cancellable ? <Button variant="danger" icon="close" onClick={() => setConfirm(true)}>{t('cgd.cancelBooking')}</Button> : !['cancelled', 'checked_out', 'no_show'].includes(booking.status) && <Link to="/app/inbox"><Button variant="secondary" icon="message" block>{t('cgd.messageDesk')}</Button></Link>}
      </div>
