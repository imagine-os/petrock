    ...
    <Page code="A-22" title="Fees & taxes" subtitle="Card service fee applied to card payments through the app (R-H03) and the tax model: one named tax with service / product / boarding rates, prices exclusive by default (R-H04, R-H05).">
      <div className="acp-two-even">
        <CrudTable<FeeRow> table="fees" title="Fees" permission="pricing.write" rowLabel={(f) => f.name}
          columns={[{ key: 'name', label: 'Fee' }, { key: 'percent', label: 'Percent', align: 'right', render: (f) => <strong>{f.percent}%</strong> }, { key: 'applies_to', label: 'Applies to', render: (f) => (f.applies_to === 'card_payments' ? 'Card payments' : 'All payments') }, { key: 'active', label: 'Status', render: (f) => activeBadge(f.active) }]}
          fields={[{ key: 'name', label: 'Name', required: true, full: true }, { key: 'kind', label: 'Kind', type: 'select', required: true, options: [{ value: 'card', label: 'Card / non-cash fee' }, { value: 'other', label: 'Other surcharge' }] }, { key: 'percent', label: 'Percent', type: 'percent', required: true, min: 0, max: 100, step: 0.01 }, { key: 'applies_to', label: 'Applies to', type: 'select', required: true, options: [{ value: 'card_payments', label: 'Card payments only' }, { value: 'all', label: 'All payments' }] }, { key: 'active', label: 'Active', type: 'toggle' }]}
