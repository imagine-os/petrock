...
export const bookingFormSpec = defineSpec({
  ...
  logic: ['Duration = package minutes by size + add-on minutes (R-G10, R-G13); default 1 h when no package (R-G17)', 'Quote via quoteGrooming (packages, addons, taxes, card fee)', 'Discount > 0 opens PinApprovalModal action discount.apply (R-X64)', 'One appointment row per pet (R-G16)', 'Status: confirmed when vaccines OK, requested otherwise', 'Capacity warning when overlaps >= capacities.grooming (R-G21)', 'New customer returns via ?return= with the new id (R-J05)'],
  ...
  rules: ['R-G01', 'R-G10', 'R-G13', 'R-G16', 'R-G17', 'R-G21', 'R-X64', 'R-J05', 'R-J06', 'R-I11', 'R-J02', 'R-J03', 'R-A11', 'R-H03', 'R-H04'], states: ['new', 'edit', 'no customer chosen', 'pet with vaccine issue', 'over capacity', 'PIN for discount', 'validation errors'],
...
export const appointmentDetailSpec = defineSpec({
  ...
  logic: ['Lines from quoteForAppointment (stored total shown when it differs)', 'Transitions from APPOINTMENT_TRANSITIONS; PIN per R-X63', 'Audit on every change (R-J08)'],
  ...
  rules: ['R-I03', 'R-X63', 'R-J08', 'R-A11', 'R-G01', 'R-H04'], states: ['not found', 'each status', 'PIN modal', 'invoice exists / preview'], figma: ['front desk-5.jpg', 'front desk-8.jpg'], checkedAt: CHECKED,
});
...
export const customerDetailSpec = defineSpec({
  ...
  logic: ['Balance = customers.balance + unpaid invoice balances', 'Deactivate = soft delete with PIN record.delete (R-X68)', 'Notes: add, pin'], integrations: [],
  ...
  rules: ['R-X68', 'R-A11', 'R-J02', 'R-I01'], states: ['not found', 'no pets', 'balance due', 'inactive customer'], checkedAt: CHECKED,
});
...
export const petFormSpec = defineSpec({
  ...
  logic: ['Required: owner, name, type, sex (R-C01)', 'Size auto from weight (working bands) unless overridden', 'Expiry defaults to vaccinated + 1 year', 'Certificate + dates by a verifier = verified; dates only = submitted; none = missing (R-X65)', 'Approval status recomputed from required vaccines'], integrations: ['File storage (later)'],
  ...
  rules: ['R-C01', 'R-C06', 'R-B03', 'R-B08', 'R-J04', 'R-J02', 'R-X65', 'R-A13'], states: ['new', 'edit', 'owner preselected (?customer=)', 'validation errors'], figma: ['Pet Details .pdf', 'Frame-1.png'], checkedAt: CHECKED,
});
export const petDetailSpec = defineSpec({
  ...
  logic: ['verifyVaccineRecord -> settleVaccineStatus: approve pet + confirm pending_vaccines bookings whose pets are all approved (R-X60)', 'Reject asks a reason and notifies the customer (R-X61)', 'Expired records flagged even when verified (R-B11)', 'Deactivate = PIN + soft delete (R-X68)'], integrations: [],
  ...
  rules: ['R-X60', 'R-X61', 'R-B01', 'R-B02', 'R-B03', 'R-B11', 'R-A05', 'R-A13', 'R-X68', 'R-J08'], states: ['not found', 'pending approval', 'approved', 'reject modal', 'bookings confirmed banner'], figma: ['Pet Details .pdf', 'Choose Vaccine.png'], checkedAt: CHECKED,
});
export const vaccineQueueSpec = defineSpec({
  ...
  logic: ['Location scope through the customer\'s home location (records are global)', 'Same verify / reject chain as F-55 (R-X60, R-X61)'], integrations: [],
  components: ['PageHeader', 'StatTile', 'Tabs', 'PetVaccineVerifyTable', 'Modal', 'Textarea', 'Button', 'Toast'], rules: ['R-X60', 'R-X61', 'R-B01', 'R-B11', 'R-A05'], states: ['empty queue', 'verified toast with confirmed bookings'], checkedAt: CHECKED,
});
export const messagesSpec = defineSpec({
  ...
  logic: [':id = invoice id, or a booking / appointment / daycare id (preview when no invoice exists)', 'Preview lines: booking.quote JSON or quoteForAppointment', 'Create invoice numbers from settings.invoice.next_number (R-X67)', 'Mark paid writes a payments row through the payment method and sets invoice + source payment_status paid (payments.write)'], integrations: ['PaymentProvider (Stripe later)'],
  components: ['PageHeader', 'DeskInvoiceSheet', 'Button', 'Badge', 'DataTable', 'EmptyState', 'Toast'], rules: ['R-X67', 'R-J06', 'R-H03', 'R-H04', 'R-H05', 'R-J03'], states: ['issued', 'paid', 'preview (not issued)', 'not found', 'print'], figma: ['front desk-5.jpg', 'front desk-8.jpg', '9.pdf'], checkedAt: CHECKED,
});
