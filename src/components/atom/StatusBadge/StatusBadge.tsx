...
/** Booking status badge; the one lifecycle vocabulary every surface uses (R-I01 colours, R-I04 customer labels). Desk `fill` = Figma 26 px r4 tinted label without a dot (598:23563, front desk-9.jpg); `pill` keeps the mobile dot. */
export function StatusBadge({ status, customer = false, size, variant = 'fill' }: StatusBadgeProps) {
  ...
  return <span className={`badge badge-status badge-${size ?? 'md'} badge-${variant}`} data-status={status} title={customer ? staff : undefined}>{variant === 'pill' && <span className="badge-dot" aria-hidden />}{label}</span>;
}
