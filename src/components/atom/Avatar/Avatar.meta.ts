...
export default defineMeta({
  ...
  props: [{ name: 'name', type: 'string', required: true, description: 'Full name (initials + label)' }, { name: 'src', type: 'string', description: 'Photo URL' }, { name: 'size', type: 'number', default: '36', description: 'Pixels' }, { name: 'kind', type: "'person'|'pet'", default: 'person', description: 'Tint scheme' }, { name: 'shape', type: "'circle'|'rounded'", default: 'circle', description: 'Shape' }],
  states: ['initials', 'photo'],
  usages: [{ title: 'Sizes and kinds', render: () => h('div', { className: 'row wrap' }, h(Avatar, { name: 'Avery Thompson', size: 48 }), h(Avatar, { name: 'Marcus Lee' }), h(Avatar, { name: 'Biscuit', kind: 'pet', size: 48, shape: 'rounded' }), h(Avatar, { name: 'Mochi', kind: 'pet' }), h(Avatar, { name: 'Bruno', kind: 'pet', size: 28 })) }],
