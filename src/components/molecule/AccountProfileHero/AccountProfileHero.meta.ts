...
export default defineMeta({
  ...
  props: [{ name: 'name', type: 'string', required: true, description: 'Display name (initials fallback)' }, { name: 'email', type: 'string | null', description: 'Shown under the name' }, { name: 'avatarUrl', type: 'string | null', description: 'Photo' }, { name: 'onPhoto', type: '(dataUrl: string) => void', description: 'Enables the camera badge' }, { name: 'maxBytes', type: 'number', default: '1500000', description: 'Reject bigger files (onTooLarge)' }, { name: 'size', type: 'number', default: '110', description: 'Avatar diameter' }, { name: 'subtitle', type: 'ReactNode', description: 'Second line override (pet: "Breed: Ragdoll")' }, { name: 'kind', type: "'person' | 'pet'", description: 'Avatar palette' }, { name: 'action', type: '{ icon, label, onClick }', description: 'Disc button when there is no photo picker (pencil -> edit pet)' }],
  ...
  usages: [
    ...
    { title: 'Pet profile', render: () => h(AccountProfileHero, { name: 'Brownie', subtitle: 'Breed: Ragdoll', kind: 'pet', action: { icon: 'edit', label: 'Edit pet', onClick: () => {} } }) },
  ],
