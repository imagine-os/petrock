...
export interface ModalProps { open: boolean; onClose: () => void; title?: ReactNode; children?: ReactNode; footer?: ReactNode; size?: 'sm' | 'md' | 'lg'; closeOnScrim?: boolean; className?: string }
  ...
  return createPortal(
    <div className="modal-scrim" onMouseDown={(e) => { if (closeOnScrim && e.target === e.currentTarget) onClose(); }}>
      <div className={`modal modal-${size} ${className}`} role="dialog" aria-modal="true" aria-label={typeof title === 'string' ? title : undefined} ref={ref} tabIndex={-1}>
        <div className="modal-head">{title && <h2 className="modal-title">{title}</h2>}<IconButton icon="close" label="Close" size="sm" onClick={onClose} /></div>
